...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        attn = torch.bmm(q, k.transpose(1, 2)) # b x n x n
...
class SelfAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        q = torch.relu(self.w_q(q)) # b x n x d
        k = torch.relu(self.w_k(k)) # b x n x d
        output = self.attention(q, k, v, mask=mask)  # b x d
        return output
...
class SelfAttentionNetwork(nn.Module):
    ...
    def forward(self, session, non_pad_mask=None, slf_attn_mask=None):
        ...
        Z = X + self.dropout(AX)
        Z *= non_pad_mask # 이건 뭐지???

        fS = self.pos_ffn(Z) # 수식 (5)
        fS *= non_pad_mask
